--MODIFICADA
    ...
    WITH raw AS (
        ...
        FROM (
            ...
            LEFT JOIN (
                        ...
                        WHERE s.partition_date >= DATE_ADD(CURRENT_DATE(), INTERVAL -2 DAY)--partition_date >= DATE_ADD('{{ next_ds }}', INTERVAL -2 DAY)
                        GROUP BY 1
    ...
    SELECT date
        ...
        ,SUM(sessions_vertical) sessions_vertical
        ,SAFE_DIVIDE (
            SUM(transaction_dummy),SUM(sessions_vertical)
        ) AS CVR
        ,SAFE_DIVIDE (
            SUM(transaction_CVR2),SUM(shop_list_dummy)
        ) AS CVR2
        ,SAFE_DIVIDE (
            SUM(transaction_CVR3),SUM(shop_details_dummy)
        ) AS CVR3
        ,SAFE_DIVIDE (
            SUM(shop_details_mCVR2),SUM(shop_list_dummy)
        ) AS mCVR2
        ,SAFE_DIVIDE (
            SUM(checkout_loaded_mCVR3),SUM(shop_details_dummy)
        ) AS mCVR3
        ,SAFE_DIVIDE (
            SUM(add_to_cart_mCVR3a),SUM(shop_details_dummy)
        ) AS mCVR3a
        ,SAFE_DIVIDE (
            SUM(checkout_loaded_mCVR3b),SUM(add_to_cart_dummy)
        ) AS mCVR3b
        ,SAFE_DIVIDE (
            SUM(checkout_loaded_mCVR3c),SUM(cart_clicked_dummy)
        ) AS mCVR3c
        ,SAFE_DIVIDE (
            SUM(transaction_mCVR4),SUM(checkout_loaded_dummy)
        ) AS mCVR4
    FROM (
        SELECT 
                date
...
--ORIGINAL
    ...
    --INSERT `peya-food-and-groceries.automated_tables_reports.fact_sessions_funnel_vertical_agg`
    ...
    SELECT date
        ...
        ,COUNT(sessions_vertical) sessions_vertical
        ,SAFE_DIVIDE (
            SUM(transaction_dummy),SUM(sessions_vertical)
        ) AS CVR
        ,SAFE_DIVIDE (
